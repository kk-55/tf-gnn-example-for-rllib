...
class Graph(Dict):
    ...
    def __init__(self, graph_spec, graph_spaces=None, seed=None, **spaces_kwargs):
        ...
        self.further_spaces = {
            name: space for name, space in self.spaces.items()
            if name not in ["node_sets", "edge_sets", "context"]
        }
    ...
    def sample(self):
        ...
        for name, space in self.further_spaces.items():
            graph[name] = space.sample()
